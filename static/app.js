...
window.newView=()=>{
 ...
 <div class="card"><h2>3 · Format</h2>${cr?`<label class="mu">Overs per innings</label><input type="number" min="1" value="${nm.ov}" oninput="nm.ov=this.value">`:`<div class="g2"><div><label class="mu">Best of (sets)</label><select onchange="nm.best=+this.value"><option${nm.best==1?' selected':''}>1</option><option${nm.best==2?' selected':''}>2</option><option${nm.best==3?' selected':''}>3</option></select></div><div><label class="mu">Points per set</label><input type="number" value="${nm.pts}" oninput="nm.pts=+this.value"></div><div><label class="mu">Deciding set points</label><input type="number" value="${nm.last}" oninput="nm.last=+this.value"></div></div>`}</div>
